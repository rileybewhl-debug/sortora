...
function payoutNotification(data) {
  ...
  return wrapper(header('Payout Sent')
    ...
    + '<div style="font-size:40px;font-weight:800;color:' + DARK + ';letter-spacing:-1.5px;line-height:1;margin-bottom:8px">$' + amt + '</div>'
...
function weeklyDigest(data) {
  ...
  return wrapper(header('Weekly Digest')
    ...
    + '<table cellpadding="0" cellspacing="0" border="0" width="100%">'
    + '<tr>'
    + '<td style="padding:20px;text-align:center;width:50%;border-bottom:1px solid ' + BORDER + '"><div style="font-size:11px;color:' + MUTED + ';text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px;font-weight:600">Revenue</div><div style="font-size:24px;font-weight:800;color:' + DARK + '">$' + rev + '</div><div style="font-size:12px;color:' + trendColor + ';font-weight:600;margin-top:4px">' + arrow + ' ' + Math.abs(pct) + '% vs last week</div></td>'
...
function paymentReminder(data) {
  ...
  return wrapper(header('Payment Reminder')
    ...
    + '<td style="font-size:18px;font-weight:800;color:' + DARK + ';text-align:right">$' + amt + '</td>'
    + '</tr></table></div>'
...
function autoChargeReceipt(data) {
  ...
  return wrapper(header('Auto-Charge Processed')
    ...
    + '<div style="background:' + GLASS + ';border-radius:12px;padding:20px 24px;margin-bottom:24px;border:1px solid ' + BORDER + '">'
    + '<table cellpadding="0" cellspacing="0" border="0" width="100%">'
    + '<tr><td style="padding:6px 0;font-size:13px;color:' + MUTED + ';font-weight:500">Auto-charged amount</td><td style="padding:6px 0;font-size:16px;color:' + DARK + ';font-weight:800;text-align:right">$' + amt + '</td></tr>'
